...
namespace Maps.Tizen.Wearable
{
	...
	class App : CoreUIApplication
    {
		...
		/// <summary>
		/// Create a provider selection page
		/// </summary>
		void CreateProviderPage()
        {
			...
			// Create a CircleSurface
			var surface = new CircleSurface(conf);

			// Create a Naviframe
			navi = new Naviframe(window);
			conf.SetContent(navi);
			navi.BackButtonPressed += (s, e) => navi.Pop();

			// Create a CircleGenList
			CircleGenList providerList = new CircleGenList(navi, surface)
			{
				Homogeneous = true,
			};
			// Create a GenItemClass for title
			GenItemClass titleClass = new GenItemClass("title")
			{
			...
			providerList.Append(titleClass, null);
			// Create a GenItemClass for providers
			GenItemClass itemClass = new GenItemClass("default")
			{
				GetTextHandler = (obj, part) =>
				{
					if (part == "elm.text")
			...
			// Create a GenItemClass for padding
			GenItemClass paddingClass = new GenItemClass("padding");
			for (int i = 0; i < MapService.Providers.Count(); i++)
			{
				providerList.Append(itemClass, MapService.Providers.ElementAt(i));
			}

			providerList.Append(paddingClass, null);
			providerList.Show();
			((IRotaryActionWidget)providerList).Activate();

			// Push the providerList to the naviframe
			var providerPage = navi.Push(providerList, null, "empty");
			providerPage.Popped += (s, e) => Exit();

			providerList.ItemSelected += (s, e) =>
			{
				view = ViewPage.MAP;
				selectedProvider = e.Item.GetPartText("elm.text");

				CreateMap(selectedProvider);
				e.Item.IsSelected = false;
			};
		}

		/// <summary>
		/// Create a MapService and MapView object.
		/// </summary>
		/// <param name="provider">Specifies the provider</param>
		public async void CreateMap(string provider)
		{
			...
			{
				// Close the Map if user doesn't agree the user consent popup
				CloseMap();
				return;
			}
