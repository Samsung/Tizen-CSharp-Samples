...
{
    class NavigationCommand<TPage> : Command
        where TPage : ContentPage, new()
    {
        ...
        public NavigationCommand(Func<object> bindingContextCreator)
          : base(async () => await Application.Current.MainPage.Navigation.PushAsync(
              new TPage() { BindingContext = bindingContextCreator?.Invoke() }))
        {
        }

        public NavigationCommand(Func<object> bindingContextCreator, Func<bool> canExecute)
          : base(async () => await Application.Current.MainPage.Navigation.PushAsync(
