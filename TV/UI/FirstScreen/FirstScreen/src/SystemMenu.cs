    ...
    public class SystemMenu
    {
        ...
        public View Show(bool show, View.FocusDirection direction)
        {
            ...
            for (int i = 0; i < _systemIcons.Length; ++i)
            {
                ...
                if (show)
                {
                    xPos += ((((Constants.SystemMenuWidth * scaleDelta) / 2.0f) - (Constants.SystemMenuWidth / 2.0f)) / scaleDelta);
                }
                targetPosition = new Position(xPos, targetPosition.Y, targetPosition.Z);
