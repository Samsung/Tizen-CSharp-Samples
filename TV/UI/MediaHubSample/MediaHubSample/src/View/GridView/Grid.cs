...
namespace Tizen.NUI
{
    ...
    public class GridView : View
    {
        ...
        {
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Set bridge for GridView"));
            ...
            for (int i = 0; i < groupList.Count; i++)
            {
                ...
                if (groupTitle != null)
                {
                    ...
                    if (GridType.Horizontal == gridType)
                    {
                        ...
                        Tizen.Log.Info("MediaHub", "titleView.PositionX: " + groupTitle.PositionX + ", titleView.PositionY: " + groupTitle.PositionY);
                    }
                    else
                    {
        ...
        {
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Add Group Number{0}", groupCount));
        ...
        {
            Tizen.Log.Info("MediaHub", "...." + groupCount);
        ...
        internal void AddColumn(int columnWidth, int groupIndex = 0)
        {
            ...
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Add column columnWidth {0} to group {1}", columnWidth, groupIndex));

            if (!IsGroupIndexValid(groupIndex))
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]Invalid group index"));
                return;
            }
        ...
        {
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Add row rowHeight {0} to column {1} of group {2}", rowHeight, columnIndex, groupIndex));
            if (!IsColumnIndexValid(groupIndex, columnIndex))
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]Invalid column index"));
                return;
            }
            ...
            Tizen.Log.Info("MediaHub", " >>>>>>> newCell.itemIndex: " + newCell.itemIndex + " newCell.rect[" + newCell.rect.X + ", " + newCell.rect.Y + ", " + newCell.rect.Width + ", " + newCell.rect.Height + "]");
            ...
            Tizen.Log.Info("MediaHub", " >>>>>>> curColumn.startItemIndex: " + curColumn.startItemIndex + " curColumn.endItemIndex: " + curColumn.endItemIndex);

            curGroup.itemIndexToCell.Add(newCell.itemIndex, newCell);
            curGroup.realCellCount++;

            Tizen.Log.Info("MediaHub", "Group: " + groupIndex + ", Column: " + columnIndex + " Cell.rowIndex: " + newCell.rowIndex + " Cell.columnIndex: " + newCell.columnIndex + " Cell.itemIndex: " + newCell.itemIndex + " curGroup.realCellCount:" + curGroup.realCellCount);
        }

        // Add row to the all column in same group of GridView.
        internal void AddRowToAllColumn(int rowHeight, int groupIndex)
        {
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Add row rowHeight {0} to to all column of group {1}", rowHeight, groupIndex));
            if (!IsGroupIndexValid(groupIndex))
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]Invalid group index"));
                return;
        ...
        {
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Add row x column : ({0} x {1}) rowHeight: {2} , columnWidth: {3} to group {4}", rowCount, columnCount, rowHeight, columnWidth, groupIndex));
        ...
        {
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Attach group title to {0}", groupIndex));
            if (!IsGroupIndexValid(groupIndex))
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]Invalid group index"));
                return;
        ...
        {
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Set Focus to item {0} of group {1}", itemIndex, groupIndex));
            if (!IsItemIndexValid(groupIndex, itemIndex))
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]Invalid item index"));
                return;
            }

            if (groupIndex == focusGroupIndex && itemIndex == focusItemIndex)
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]Already focused"));
                return;
            }
            //if (IsFocused == true)
            //{
        ...
        {
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Move to next page"));
            PageMove(false, bAni);
        }
        ...
        {
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Move to previous page"));
            PageMove(true, bAni);
        }
        ...
        {
            Tizen.Log.Info("MediaHub", string.Format("[GridView]Update item {0} of group {1}", itemIndex, groupIndex));
            if (!IsItemIndexValid(groupIndex, itemIndex))
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]Invalid group index"));
                return;
        ...
        {
            Tizen.Log.Info("MediaHub", "... groupIndex = " + groupIndex);
            if (!IsGroupIndexValid(groupIndex))
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]Invalid group index"));
            ...
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]Invalid column list"));
                return INVALID_COUNT;
            }

            Tizen.Log.Info("MediaHub", "... curGroup.columnList.Count = " + curGroup.columnList.Count);
        ...
        protected override void Dispose(DisposeTypes type)
        {
            ...
            if (type == DisposeTypes.Explicit)
            {
                ...
                Tizen.Log.Info("MediaHub", string.Format("[GridView]Dispose"));
                focusMoveAni?.Stop();
        ...
        protected void OnRelayoutUpdate(object sender, EventArgs e)
        {
            ...
            Tizen.Log.Info("MediaHub", "OnRelayoutUpdate()");
            if (gridBridge != null)
            {
                Load();
        ...
        private void Load()
        {
            ...
            UpdateInMemoryItems();
            Tizen.Log.Info("MediaHub", ">>>>>>>>> itemGroupRect: " + itemGroupRect.X + ", " + itemGroupRect.Y + ", " + itemGroupRect.Width + ", " + itemGroupRect.Height);
            Tizen.Log.Info("MediaHub", ">>>>>>>>> itemGroup: " + itemGroup.PositionX + ", " + itemGroup.PositionY + ", " + itemGroup.SizeWidth + ", " + itemGroup.SizeHeight);
        }
        ...
        private void UpdateInMemoryItems()
        {
            ...
            Tizen.Log.Info("MediaHub", ">>>>>>>>>  loadItemList.Count: " + loadItemList.Count + " unloadItemList.Count: " + unloadItemList.Count);
        ...
        {
            Tizen.Log.Info("MediaHub", "item: " + item + " bDirectly: " + bDirectly);
        ...
        private void LoadItem(GridItem item)
        {
            ...
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]LoadItem item view is null"));
                return;
            }

            View temp = GetRecycleView(item.groupIndex,item.index);
            Tizen.Log.Info("MediaHub", " temp: " + temp);
            if (temp != null)
            {
                item.itemView = temp;
                gridBridge.UpdateItem(item.groupIndex, item.index, item.itemView);
            ...
            Tizen.Log.Info("MediaHub", "item.index: " + item.index + " item.rect.X: " + item.rect.X + " item.rect.Y: " + item.rect.Y);
        ...
        private void SetViewTypeCount(int typeCount)
        {
            if (typeCount < 1)
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]SetViewTypeCount type count less than 1"));
                return;
            }
        ...
        private void AddRecycleView(int groupIndex, int itemIndex, View view)
        {
            ...
            {
                Tizen.Log.Info("MediaHub", string.Format("[GridView]AddRecycleView viewType larger than viewTypeCount"));
                return;
            }
        ...
        {
            int viewType = gridBridge.GetItemType(groupIndex, itemIndex);
            Tizen.Log.Info("MediaHub", " viewType:" + viewType + " viewTypeCount:" + viewTypeCount);
        ...
        {
            Tizen.Log.Info("MediaHub", ">>>>>>>>>  aLastVisibleArea.X: " + aLastVisibleArea.X + " aLastVisibleArea.Y: " + aLastVisibleArea.Y + " aLastVisibleArea.Width: " + aLastVisibleArea.Width + " aLastVisibleArea.Height: " + aLastVisibleArea.Height);
            Tizen.Log.Info("MediaHub", ">>>>>>>>>  curVisibleArea.X: " + curVisibleArea.X + " curVisibleArea.Y: " + curVisibleArea.Y + " curVisibleArea.Width: " + curVisibleArea.Width + " curVisibleArea.Height: " + curVisibleArea.Height);
        ...
        {
            Tizen.Log.Info("MediaHub", ">>>>>>>>> direction: " + direction + " fromGroupIndex: " + fromGroupIndex + " fromItemIndex: " + fromItemIndex);
            ...
            switch (direction)
            {
                ...
                case "Left":
                    ...
                    else
                    {
                        ...
                        Tizen.Log.Info("MediaHub", ">>>>>>>>> destFocusGroupIndex: " + destFocusGroupIndex + " destFocusColumnIndex:" + destFocusColumnIndex + " currentCell.rowIndex: " + currentCell.rowIndex);
                        Column nextColumn = currentGroup.columnList.ElementAt(destFocusColumnIndex);
                        Cell nextCell = nextColumn.cellList.ElementAt(currentCell.rowIndex);
                        destFocusItemIndex = nextCell.itemIndex;
                        Tizen.Log.Info("MediaHub", ">>>>>>>>> destFocusItemIndex: " + destFocusItemIndex);
                    }
                ...
                case "Right":
                    ...
                    else
                    {
                        ...
                        if (currentCell.rowIndex >= nextColumn.cellList.Count)
                        {
                            ...
                            Cell nextCell = nextColumn.cellList.ElementAt(nextColumn.cellList.Count - 1);
                            destFocusItemIndex = nextCell.itemIndex;
                            Tizen.Log.Info("MediaHub", ">>>>>>>>> destFocusItemIndex: " + destFocusItemIndex + " currentCell.rowIndex: " + currentCell.rowIndex + " nextColumn.cellList.Count: " + nextColumn.cellList.Count);
                        }
                        else
                        {
                            Cell nextCell = nextColumn.cellList.ElementAt(currentCell.rowIndex);
            ...
            Tizen.Log.Info("MediaHub", ">>>>>>>>> destFocusGroupIndex: " + destFocusGroupIndex + " destFocusColumnIndex:" + destFocusColumnIndex + " destFocusItemIndex: " + destFocusItemIndex);
            GridItem item = null;
        ...
        private void MoveFocus(string direction, int fromGroupIndex, int fromItemIndex)
        {
            ...
            switch (direction)
            {
                ...
                case "Right":
                    ...
                    Tizen.Log.Info("MediaHub", ">>>>>>>>> focusedColumnIndex: " + focusedColumnIndex);
                    ...
                    else
                    {
                        ...
                        Tizen.Log.Info("MediaHub", ">>>>>>>>> destFocusGroupIndex: " + destFocusGroupIndex + " destFocusColumnIndex:" + destFocusColumnIndex);
                        GetNearestItem(curFocusedCell, destFocusGroupIndex, destFocusColumnIndex, ref destFocusItemIndex, true);
                        Tizen.Log.Info("MediaHub", ">>>>>>>>> destFocusItemIndex: " + destFocusItemIndex);
        ...
        {
            Tizen.Log.Info("MediaHub", ">>>>>>>>> groupIndex: " + groupIndex + " itemIndex: " + itemIndex);
            if (!IsItemIndexValid(groupIndex, itemIndex))
            {
                return;
            }
            ...
            {
                Position itemGroupPos = null;
                bNeedScroll = GetItemGroupPosByFocusItem(groupIndex, itemIndex, out itemGroupPos);
            ...
            Tizen.Log.Info("MediaHub", ">>>>>>>>> bNeedScroll: " + bNeedScroll + " itemGroupRect.X: " + itemGroupRect.X + " itemGroupRect.Y: " + itemGroupRect.Y);
            ResetItemGroupRectPos();

            Tizen.Log.Info("MediaHub", ">>>>>>>>> bNeedScroll: " + bNeedScroll + " itemGroupRect.X: " + itemGroupRect.X + " itemGroupRect.Y: " + itemGroupRect.Y);
            if (bNeedScroll)
            {
                Tizen.Log.Info("MediaHub", ">>>>>>>>> bAnimation: " + bAnimation);
                if (bAnimation == true)
                {
        ...
        private void PushFocus(int groupIndex, int itemIndex)
        {
            ...
            Position itemGroupPos = null;
            bool bNeedScroll = GetItemGroupPosByFocusItem(groupIndex, itemIndex, out itemGroupPos);

            itemGroupRect.X = itemGroupPos.X;
            itemGroupRect.Y = itemGroupPos.Y;
            Tizen.Log.Info("MediaHub", ">>>>>>>>> itemGroupRect.X: " + itemGroupRect.X + " itemGroupRect.Y:" + itemGroupRect.Y);
            ...
            if (bNeedScroll)
            {
                ...
                FocusMoveViaObject obj = new FocusMoveViaObject();
                Tizen.Log.Info("MediaHub", ">>>>>>>>> focusMoveViaQ.Count: " + focusMoveViaQ.Count);
                Tizen.Log.Info("MediaHub", "::::: " + focusGroupIndex + ", " + focusItemIndex);
            ...
            else
            {
                ...
                {
                    Tizen.Log.Info("MediaHub", "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ " + groupIndex + " " + itemIndex + " ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
                    NotifyFocusChange(focusGroupIndex, focusItemIndex, groupIndex, itemIndex);
                }
            }
        ...
        private bool GetItemGroupPosByFocusItem(int aFocusGroupIndex, int aFocusItemIndex, out Position itemGroupPos)
        {
            bool bNeedScroll = false;
            if (!IsItemIndexValid(aFocusGroupIndex, aFocusItemIndex))
            {
                itemGroupPos = new Position(0, 0, 0);
                return false;
            }
            ...
            Tizen.Log.Info("MediaHub", ">>>>>>>>> itemGroupRect: " + itemGroupRect.X + ", " + itemGroupRect.Y + ", " + itemGroupRect.Width + ", " + itemGroupRect.Height);
            Tizen.Log.Info("MediaHub", ">>>>>>>>> itemGroup: " + itemGroup.PositionX + ", " + itemGroup.PositionY + ", " + itemGroup.SizeWidth + ", " + itemGroup.SizeHeight);
            Tizen.Log.Info("MediaHub", ">>>>>>>>> focusedItem.rect: " + focusedItem.rect.X + ", " + focusedItem.rect.Y + ", " + focusedItem.rect.Width + ", " + focusedItem.rect.Height);
            ...
            float posX = itemGroupRect.X;
            float posY = itemGroupRect.Y;
            ...
            if (GridType.Horizontal == gridType)
            {
                ...
                {
                    Tizen.Log.Info("MediaHub", "Left  aFocusItemIndex:: " + aFocusGroupIndex + ", aFocusItemIndex:: " + aFocusItemIndex);
                    GridItem prevItem = GetItemByDirection("Left", aFocusGroupIndex, aFocusItemIndex);
                    //Tizen.Log.Info("MediaHub", "startFocusRange :: " + margin[0]);
                ...
                else
                {
                    ...
                    //Tizen.Log.Info("MediaHub", "gridWidth::" + gridWidth + "  gridWidth - margin[2] :: " + (gridWidth - margin[2]));
                    //Tizen.Log.Info("MediaHub", "gridWidth - nextItem.rect.Width / 2 :: " + (gridWidth - nextItem.rect.Width / 2));
                    endFocusRange = nextItem != null ? gridWidth - nextItem.rect.Width / 2 : gridWidth - margin[2];
                    Tizen.Log.Info("MediaHub", "endFocusRange :: " + endFocusRange);
                }

                if (focusBarRectBaseOnList.X < startFocusRange)
                {
                    Tizen.Log.Info("MediaHub", "focusBarRectBaseOnList.X :: " + focusBarRectBaseOnList.X + "  startFocusRange :: " + startFocusRange);
                    posX += startFocusRange - focusBarRectBaseOnList.X;
                    bNeedScroll = true;
                }
                else if (focusBarRectBaseOnList.Right() > endFocusRange)
                {
                    Tizen.Log.Info("MediaHub", "focusBarRectBaseOnList.Right() :: " + focusBarRectBaseOnList.Right() + "  endFocusRange :: " + endFocusRange);
                    posX += endFocusRange - focusBarRectBaseOnList.Right();
                    bNeedScroll = true;
                }
            }
            else
            {
                ...
                {
                    posY += startFocusRange - focusBarRectBaseOnList.Y;
                    bNeedScroll = true;
                }
                else if (focusBarRectBaseOnList.Bottom() > endFocusRange)
                {
                    posY += endFocusRange - focusBarRectBaseOnList.Bottom();
                    bNeedScroll = true;
                }
            }

            itemGroupPos = new Position(posX, posY, 0);
        ...
        {
            Tizen.Log.Info("MediaHub", ">>>>>>>>> fromGroupIndex: " + fromGroupIndex + " fromItemIndex: " + fromItemIndex + " toGroupIndex: " + toGroupIndex + " toItemIndex: " + toItemIndex);
            ...
            if (IsItemIndexValid(toGroupIndex, toItemIndex))
            {
                ...
                if (item.itemView != null)
                {
                    Tizen.Log.Info("MediaHub", "Call GridBridge.FocusChange to set focus");
            ...
            Tizen.Log.Info("MediaHub", ">>>>>>>>> focusGroupIndex: " + focusGroupIndex + " focusItemIndex: " + focusItemIndex);
        }
        ...
        {
            Tizen.Log.Info("MediaHub", "--------------------- " + focusMoveViaQ.Count + " -------------------------------");
        ...
        {
            Tizen.Log.Info("MediaHub", "--------------------- " + groupIndex + " -------------------------------");
            Group curGroup = groupList.ElementAt(groupIndex);
        ...
        private void ResetByData()
        {
            ...
            for (int i = 0; i < groupCount; i++)
            {
                Tizen.Log.Info("MediaHub", ".... group: " + i);
                int dataCount = gridBridge.GetItemCount(i);
                Tizen.Log.Info("MediaHub", ".... dataCount: " + dataCount + " rowCnt: " + rowCnt);
                int columnCount = (dataCount + rowCnt - 1) / rowCnt;
                Group group = groupList.ElementAt(i);
                Tizen.Log.Info("MediaHub", ".... columnCount: " + columnCount + " group.columnList.Count: " + group.columnList.Count + " group.itemIndexToCell.Count: " + group.itemIndexToCell.Count);
                ...
                else if (columnCount < group.columnList.Count)
                {
                    ...
                    for (int c = columnCount; c < group.columnList.Count; c++)
                    {
                        for (int r = 0; r < rowCnt; r++)
                        {
                            ...
                            group.realCellCount--;
                            Tizen.Log.Info("MediaHub", ".... itemIndexToCell.Remove: " + itemIndex);
                        }
                    }
                ...
                Tizen.Log.Info("MediaHub", ".... group.columnList.Count: " + group.columnList.Count);
            }
        ...
        private class GridItem
        {
            public void ScaleItem(Vector3 scaleFactor, AnimationAttributes aniAttrs)
            {
                ...
                {
                    scaleAni.EndAction = Animation.EndActions.StopFinal;
                }
    ...
    public abstract class GridBridge
    {
        ...
        public void Remove(int itemIndex, int groupIndex = 0, bool withAni = false, bool sendEvent = true)
        {
            ...
            {
                Tizen.Log.Info("MediaHub", "Error!! item index is invalid");
                return;
            }
