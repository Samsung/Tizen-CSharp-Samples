...
namespace Tizen.NUI.MediaHub
{
    ...
    public class FootView : View
    {
        ...
        public void Update(FootModel footModel)
        {
            ...
            if (itemType == ContentItemType.eItemPhoto || itemType == ContentItemType.eItemVideo)
            {
                ...
                for (int i = 0; i < Itemlist.Count; i++)
                {
                    ...
                    informationTextWdg[0].Size2D = new Size2D((int)width, informationTextWdg[0].NaturalSize2D.Height);
                    Tizen.Log.Info("MediaHub", " FootView Update informationTextWdg[0].GetNaturalSize().Width = " + 0 + " " + informationTextWdg[0].NaturalSize2D.Width);
        ...
        {
            Tizen.Log.Info("MediaHub", "[FootView]createInformation!");
        ...
        {
            //To Create a bg view
            Tizen.Log.Info("MediaHub", "[FootView CreateUSBButton!]");
        ...
        private void OnButtonClicked(object sender, Dropdown.ItemSelectedEventArgs e)
        {
            ...
            else if (button.Name == "SortBy")
            {
                int index = e.SelectedItemIndex;
                GridListView preGridList = gridListView;
                Tizen.Log.Info("MediaHub", "DropDown.ItemSelectedEvent, index = " + index);
                switch (index)
                {
                    case 0:
