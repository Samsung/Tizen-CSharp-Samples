...
*/
        ...
        private void DeleteItem(int fromItemIndex, int deleteItemNum)
        {
            ...
            if (headItemIndex <= toItemIndex && tailItemIndex >= fromItemIndex)
            {
                ...
                float itemGroupPosY = 0.0f;
                bool flagScrollItem = CalItemGroupPosByFocusItemIndex(curFocusItemIndex, out itemGroupPosY);
                itemGroupRect.Y = itemGroupPosY;
        ...
        private void MoveFocus(int focusItemIndex)
        {
            ...
            float itemGroupPosY = 0.0f;
            bool flagScrollItem = CalItemGroupPosByFocusItemIndex(focusItemIndex, out itemGroupPosY);

            itemGroupRect.X = 0;
            itemGroupRect.Y = (int)itemGroupPosY;
        ...
        private bool CalItemGroupPosByFocusItemIndex(int focusIndex, out float itemGroupPosY)
        {
            ...
            itemGroupPosY = itemGroupRect.Y;
            if (destItemRectOnList.Y < startFocusRange)
            {
                needScroll = true;
                itemGroupPosY += startFocusRange - destItemRectOnList.Y;
            }
            else if (destItemRectOnList.Bottom() > endFocusRange)
            {
                needScroll = true;
                itemGroupPosY += endFocusRange - destItemRectOnList.Bottom();
            }
            ...
            if (tailItemIndex == itemList.Count - 1)
            {
                if (headItemIndex == 0)
                {
                    itemGroupPosY = margin[1];
                }
                else
                {
                    itemGroupPosY = this.SizeHeight - margin[3] - itemGroupRect.Height;
                }
            }

            Tizen.Log.Debug("NUI", "itemGroupPosY : " + itemGroupPosY + "margin[1] : " + margin[1]);
            return needScroll;
        }
